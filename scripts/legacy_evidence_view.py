...
It is not a verifier. It replaces `verify_evidence.sh`, `finalize_collection.py`,
`evidence_profile.py`, `build_evidence_envelope.py` and `tool_identity.py`, and it
does not inherit their job. It does not decide whether a retained record
qualifies anything, does not maintain an anchor file or a retraction registry,
and has no notion of an active record. `evidence/ANCHORS` and
`evidence/RETRACTIONS.json` are retained bytes like any others: they are read
without being modified and they are not interpreted.
...
It is not a translator of last resort. This repository's 6 retained envelopes
...
def retained_envelopes() -> list[Path]:
    paths = sorted(EVIDENCE.glob("tl-mltl-v01-*/evidence-envelope.json"))
    if not paths:
...
def retained_report(mapper: Callable[..., dict[str, Any]]) -> dict[str, Any]:
    ...
    return {
        ...
        "statement": (
            ...
            "pass and not converted into a defect of these records. The answer was measured "
            "here rather than inherited: quire-analyze's retained summaries read 'unreadable' "
            "instead, and quire-contract-ir's real PGM-01 records map. Filed upstream as "
...
def census(mapper: Callable[..., dict[str, Any]]) -> dict[str, Any]:
    ...
    return {
        "schemaVersion": "tl-mltl.legacy-compatibility-census/v1",
        "mapping_authority": (
            "engineering_assurance.verification_semantics.map_pgm01_bytes from the pinned "
