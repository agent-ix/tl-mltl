...
def tagged_test_names(root: Path) -> set[str]:
    ...
    for source in root.rglob("*.rs"):
        ...
        if relative.parts and relative.parts[0] in {".git", "target", ".qualification-target"}:
            continue
...
def git_tagged_test_names(root: Path, revision: str) -> set[str]:
    ...
    names: set[str] = set()
    for path in paths:
        if not path.endswith(".rs") or path.startswith(("target/", ".qualification-target/")):
            continue
...
def cargo_list(cargo: str, ignored: bool = False) -> str:
...
def main() -> int:
    ...
    try:
        ...
        observed, _ = listed_test_names(cargo_list("cargo"))
        ignored, qualified_ignored = listed_test_names(cargo_list("cargo", ignored=True))
