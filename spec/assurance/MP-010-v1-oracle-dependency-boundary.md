...
definition_version: tl.v1.v1-oracle-dependency-boundary/v1
...
objective:
  ...
  bound: 1
statistical_design:
  population: one exact direct native invocation for V1.oracle_dependency_boundary in the five-source Stage 1 campaign
...
The exact command and bound response are in `campaign/procedures/v1-oracle-dependency-boundary.json`. EA runs the command without a shell. Quoin retains its request, raw stdout and stderr, output artifacts, collection, and checker result. The TL Rust checker independently scores the decisive native facts.
