---
id: MP-014
title: TL Stage 1 V2 finite small partition native result
type: MeasurementPlan
status: active
owner: tl-mltl-evidence-owner
metric: tl.v1.v2_finite_small_partition.passed
definition_version: tl.v1.v2-finite-small-partition/v1
...
objective:
  ...
  bound: 1
statistical_design:
  population: one exact direct native invocation for V2.finite_small_partition in the five-source Stage 1 campaign
  ...
  decision_rule:
    ...
    threshold: 1
protected_apparatus:
  - campaign/procedures/v2-finite-small-partition.json
...
# TL Stage 1 V2 finite small partition native result

## Decision Use

This direct native result is one required member of the V2 Campaign group. It informs the bounded Stage 1 evidence decision and does not authorize a release.
...
The exact command and bound response are in `campaign/procedures/v2-finite-small-partition.json`. EA runs the command without a shell. Quoin retains its request, raw stdout and stderr, output artifacts, collection, and checker result. The TL Rust checker independently scores the decisive native facts.
