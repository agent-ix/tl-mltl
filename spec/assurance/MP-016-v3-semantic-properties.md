---
id: MP-016
title: TL Stage 1 V3 semantic properties native result
type: MeasurementPlan
status: active
owner: tl-mltl-evidence-owner
metric: tl.v1.v3_semantic_properties.passed
definition_version: tl.v1.v3-semantic-properties/v1
execution_procedure: campaign/procedures/v3-semantic-properties.json
stage: gate
...
objective:
  ...
  bound: 1
statistical_design:
  population: one exact direct native invocation for V3.semantic_properties in the five-source Stage 1 campaign
  ...
  decision_rule:
    ...
    threshold: 1
protected_apparatus:
  - campaign/procedures/v3-semantic-properties.json
...
# TL Stage 1 V3 semantic properties native result

## Decision Use

This direct native result is one required member of the V3 Campaign group. It informs the bounded Stage 1 evidence decision and does not authorize a release.
...
The exact command and bound response are in `campaign/procedures/v3-semantic-properties.json`. EA runs the command without a shell. Quoin retains its request, raw stdout and stderr, output artifacts, collection, and checker result. The TL Rust checker independently scores the decisive native facts.
