---
id: MP-021
title: TL Stage 1 V4.syntax_infinite_wire_decode native libFuzzer result
type: MeasurementPlan
status: active
owner: tl-mltl-evidence-owner
metric: tl.v4.syntax_infinite_wire_decode.bounded_no_crash
definition_version: tl.v4.syntax-infinite-wire-decode/v1
execution_procedure: campaign/procedures/v4-syntax-infinite-wire-decode.json
stage: gate
...
objective:
  ...
  bound: 1
statistical_design:
  population: one exact direct native libFuzzer invocation for V4.syntax_infinite_wire_decode in the five-source Stage 1 campaign
  ...
  decision_rule:
    ...
    threshold: 1
protected_apparatus:
  - campaign/procedures/v4-syntax-infinite-wire-decode.json
...
# TL Stage 1 V4.syntax_infinite_wire_decode native libFuzzer result
...
The direct command and response are in `campaign/procedures/v4-syntax-infinite-wire-decode.json`. EA runs the command without a shell in a complete exact Git source projection. Quoin retains the request, raw stdout and stderr, and optional `crash.bin` output artifact. The TL Rust checker independently scores the engine budget and crash evidence.
