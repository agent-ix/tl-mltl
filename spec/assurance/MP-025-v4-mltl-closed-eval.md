---
id: MP-025
title: TL Stage 1 V4.mltl_closed_eval native libFuzzer result
type: MeasurementPlan
status: active
owner: tl-mltl-evidence-owner
metric: tl.v4.mltl_closed_eval.bounded_no_crash
definition_version: tl.v4.mltl-closed-eval/v1
...
objective:
  ...
  bound: 1
statistical_design:
  population: one exact direct native libFuzzer invocation for V4.mltl_closed_eval in the five-source Stage 1 campaign
  ...
  decision_rule:
    ...
    threshold: 1
protected_apparatus:
  - campaign/procedures/v4-mltl-closed-eval.json
...
# TL Stage 1 V4.mltl_closed_eval native libFuzzer result
...
The direct command and response are in `campaign/procedures/v4-mltl-closed-eval.json`. EA runs the command without a shell in a complete exact Git source projection. Quoin retains the request, raw stdout and stderr, and optional `crash.bin` output artifact. The TL Rust checker independently scores the engine budget and crash evidence.
