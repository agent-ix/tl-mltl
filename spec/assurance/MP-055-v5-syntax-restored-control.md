---
id: MP-055
title: TL Stage 1 V5 tl-syntax restored-control native mutation result
type: MeasurementPlan
status: active
owner: tl-mltl-evidence-owner
metric: tl.v5.syntax_restored_control.verified
definition_version: tl.v5.syntax-restored-control/v1
execution_procedure: campaign/procedures/v5-syntax-restored-control.json
stage: gate
...
objective:
  ...
  bound: 1
statistical_design:
  population: one exact direct native restored-control invocation for tl-syntax in the Stage 1 campaign
  ...
  decision_rule:
    ...
    threshold: 1
protected_apparatus:
  - campaign/procedures/v5-syntax-restored-control.json
...
# TL Stage 1 V5 tl-syntax restored-control native mutation result
...
The exact reviewed mutant selection for `tl-syntax` from the V5 source population. Discovery, mutation, and restored-green control each have their own EA attempt and independent TL checker receipt.

## Collection Procedure

The direct command and bounded response are in `campaign/procedures/v5-syntax-restored-control.json`. EA executes in a complete exact Git source projection. Quoin retains the request, raw process streams, and mutation output tree when applicable. The TL Rust checker independently scores selected native outcomes against retained discovery and source-bound reviews.
