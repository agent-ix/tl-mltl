---
id: MP-059
title: TL Stage 1 V5 tl-rewrite discovery native mutation result
type: MeasurementPlan
status: active
owner: tl-mltl-evidence-owner
metric: tl.v5.rewrite_discovery.verified
definition_version: tl.v5.rewrite-discovery/v1
execution_procedure: campaign/procedures/v5-rewrite-discovery.json
stage: gate
...
objective:
  ...
  bound: 1
statistical_design:
  population: one exact direct native discovery invocation for tl-rewrite in the Stage 1 campaign
  ...
  decision_rule:
    ...
    threshold: 1
protected_apparatus:
  - campaign/procedures/v5-rewrite-discovery.json
...
# TL Stage 1 V5 tl-rewrite discovery native mutation result
...
The exact reviewed mutant selection for `tl-rewrite` from the V5 source population. Discovery, mutation, and restored-green control each have their own EA attempt and independent TL checker receipt.

## Collection Procedure

The direct command and bounded response are in `campaign/procedures/v5-rewrite-discovery.json`. EA executes in a complete exact Git source projection. Quoin retains the request, raw process streams, and mutation output tree when applicable. The TL Rust checker independently scores selected native outcomes against retained discovery and source-bound reviews.
