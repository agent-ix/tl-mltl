---
id: MP-125
title: TL Stage 1 V4.mltl_wire_cli_decode native libFuzzer result
type: MeasurementPlan
status: active
owner: tl-mltl-evidence-owner
metric: tl.v4.mltl_wire_cli_decode.bounded_no_crash
definition_version: tl.v4.mltl-wire-cli-decode/v1
...
objective:
  ...
  bound: 1
statistical_design:
  population: one exact direct native libFuzzer invocation for V4.mltl_wire_cli_decode in the five-repository Stage 1 campaign
  ...
  decision_rule:
    ...
    threshold: 1
protected_apparatus:
  - campaign/procedures/v4-mltl-wire-cli-decode.json
...
# TL Stage 1 V4.mltl_wire_cli_decode native libFuzzer result
...
One direct EA invocation of the declared `wire_cli_decode` libFuzzer target at the exact five-repository source graph. The metric is one only when 1,000 executions with seed 229, address sanitizer, a clean exit, and no crash artifact are independently verified; zero otherwise.

## Collection Procedure

This TL-229 target exercises the real CLI command decode boundary. The direct command and response are in `campaign/procedures/v4-mltl-wire-cli-decode.json`. EA runs the command without a shell in a complete exact Git source projection. Quoin retains the request, raw stdout and stderr, and optional `crash.bin` output artifact. The TL Rust checker independently scores the engine budget and crash evidence.
