---
id: MP-126
title: TL Stage 1 V4.mltl_trace_history_intake native libFuzzer result
type: MeasurementPlan
status: active
owner: tl-mltl-evidence-owner
metric: tl.v4.mltl_trace_history_intake.bounded_no_crash
definition_version: tl.v4.mltl-trace-history-intake/v1
execution_procedure: campaign/procedures/v4-mltl-trace-history-intake.json
stage: gate
...
# TL Stage 1 V4.mltl_trace_history_intake native libFuzzer result
...
One direct EA invocation of the declared `trace_history_intake` libFuzzer target at the exact five-repository source graph. The metric is one only when 1,000 executions with seed 230, address sanitizer, a clean exit, and no crash artifact are independently verified; zero otherwise.

## Collection Procedure

This TL-229 target exercises the strict trace and history intake boundary. The direct command and response are in `campaign/procedures/v4-mltl-trace-history-intake.json`. EA runs the command without a shell in a complete exact Git source projection. Quoin retains the request, raw stdout and stderr, and optional `crash.bin` output artifact. The TL Rust checker independently scores the engine budget and crash evidence.
