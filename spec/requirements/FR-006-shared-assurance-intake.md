...
| FR-006-AC-7 | No repository-local generic runner, evidence envelope, manifest, identity framework, retention store, audit store, anchor verifier, failure-propagation policer, or aggregate verdict remains in the execution path, and neither the retained legacy evidence nor any reader, fixture set, or schema belonging to it is present or referenced. | Test (TC-024) |
