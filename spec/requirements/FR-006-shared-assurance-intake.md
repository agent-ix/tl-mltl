...
| FR-006-AC-7 | No repository-local generic runner, evidence envelope, manifest, identity framework, retention store, audit store, anchor verifier, failure-propagation policer, compatibility view, or aggregate verdict remains in the execution path. Neither the retained legacy evidence nor any reader, fixture set, or schema belonging to it is present or referenced. The removal census enumerates tracked and untracked-not-ignored repository paths through Git, compares its exact denials and deleted-reference needles with the change declaration, constrains exact tracked per-area cardinalities and declaration exemptions, scans raw bytes, rejects renamed legacy-compatibility fixture paths and obligations, and fails closed with a named diagnostic when it cannot enumerate or read. | Test (TC-024) |
