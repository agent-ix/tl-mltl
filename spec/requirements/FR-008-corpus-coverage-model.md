...
|---|---|---|
| FR-008-AC-1 | Every tuple in the reviewed v1 cell-obligation registry appears exactly once as applicable, excluded, or blocked under the specified tuple preimage and ordered-set identity; every required dimension class appears, and deleting, duplicating, reordering, reclassifying, or adding an unknown value makes the census fail while naming the complete conflicting cell set. | Test (TC-091, TC-092) |
| FR-008-AC-2 | Every applicable cell has exactly one canonical fixture and independent expected result, every excluded cell has a stable reason, and every blocked cell has an unresolved dependency and no verdict. | Test (TC-092, TC-093) |
| FR-008-AC-3 | Interval and observation partitions exercise every stated minimum, maximum, short/exact/long, witness/counterexample, progress, malformed, and resource boundary without treating a representative fixture as exhaustive value coverage. | Test (TC-093, TC-094, TC-095) |
| FR-008-AC-4 | Current W/M cells compare source lowering with the byte-identical canonical graph without adding evaluator semantics; past cells remain blocked and bind anchor/history when admitted; mixed-time and incompatible-profile cells remain excluded under their exact profile revisions. | Test (TC-096, TC-097, TC-101) |
| FR-008-AC-5 | A conformance replay consumes stored expected results and refuses an oracle that calls any owning production path, stale oracle identity, missing rejection stage/reason, or a generated campaign input presented as a canonical fixture. | Test (TC-093, TC-099) |
...
implementation. The dimension catalog admits no native-predicate or
native-temporal correspondence class: that coverage dimension is `quire-mltl`'s
under ADR-002, and a cell naming one is an unknown-dimension refusal here.
