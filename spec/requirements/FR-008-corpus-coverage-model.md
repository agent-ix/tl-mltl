...
|---|---|
| formula/operator | constants, proposition, Not, And, Or, F, G, U, R; conditional canonical-lowering pairs for W/M; conditional O/H/S/T; incompatible mixed-time graph |
...
Each cell is unique by the ordered tuple of campaign identity plus every
dimension value. Its `cellSha256` is the SHA-256 of that canonical tuple with
domain separator `tl-mltl.coverage-cell/v1`; lifecycle state is excluded from
the key so a blocked-to-applicable transition cannot disguise the same cell as
a new concern. Each record also names the owning fixture family, the required
oracle/evidence method, dependency revisions, and one state:
...
`[0,0]`. W/M cells bind their source notation and exact primitive graph but do
not create a second evaluator-semantic cell. Past cells bind an anchor and
position-history class. Mixed future/past cells are excluded unless a later
reviewed profile admits them.
...
|---|---|---|
| FR-008-AC-1 | Every tuple in the closed v1 dimension catalog appears exactly once as applicable, excluded, or blocked under its canonical tuple-derived cell identity; deleting, duplicating, reclassifying, or adding an unknown value makes the census fail while naming the complete conflicting cell set. | Test (TC-037, TC-038) |
...
| FR-008-AC-4 | W/M cells compare source lowering with the byte-identical canonical graph without adding evaluator semantics; past cells bind anchor/history; mixed-time and incompatible-profile cells remain excluded or blocked under their exact profile revisions. | Test (TC-042, TC-043, TC-047) |
...
Current future-v1 cells depend on FR-001 through FR-003. W/M admission depends
on accepted tl-syntax #37. Past/history admission depends on accepted tl-syntax
#38 and its routed tl-mltl implementation. Native predicate cells remain
blocked on quire-contract-ir #63 and native temporal bridge cells on #63/#64.
