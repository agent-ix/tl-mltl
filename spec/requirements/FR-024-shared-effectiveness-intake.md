...
- Exact MRS-003 campaign identity and one FR-020 through FR-023 domain record.
...
The production-symbol relation required by FR-022 is the Quire `implements`
export that `agent-ix/quire-rs#171` delivered; it is scope and never backs an
acceptance criterion. Each mutation population must first demonstrate complete
applicable bindings at its exact revision and exact tool identity, as an
executable control — measured at this head, the installed `quire 0.32.2`
reports zero bound edges for this repository's six `Implements:` annotations,
so the completeness of the relation cannot be assumed from the annotations
being present. No producer may infer or maintain a private substitute.

The current Quoin 0.23.1 cargo-mutants evidence adapter is insufficient as the sole
FR-022 intake because it emits a per-function score while discarding atomic
mutant identities, survivor dispositions, environment, and the complete raw
report. The FR-022 producer shall instead populate Quoin MeasurementCollection
...
|---|---|---|
| FR-024-AC-1 | Rust domain producers, Quire static export, Quoin MeasurementCollection v2 wrapper/intake/retention, and Engineering Assurance compatibility each perform only their allocated role, and process-spawn probes prove Quire/Quoin do not execute campaign or external producers. | Test (TC-125, TC-126) |
| FR-024-AC-2 | Every domain state maps to the exact shared ownership-registry state without erasure; absent, partially written, stale, suspect, vacuous, tampered, unreadable, or digest-mismatched producer bytes refuse and cannot be inferred from exit status, logs, counts, or an older result. | Test (TC-126, TC-127) |
| FR-024-AC-3 | Every source, requirement, test/harness/target, tool, dependency, configuration, environment, raw artifact, status, limitation, owner, and retention identity is bound by exact digest or typed identity before a result is credited. | Test (TC-121, TC-127) |
| FR-024-AC-4 | Every FR-009 path/digest/resource boundary and any tighter accepted shared bound refuses before bytes are decoded or allocated, and an out-of-tree binary artifact remains blocked unless an accepted Quoin capability binds immutable storage identity and content digest. | Test (TC-128) |
| FR-024-AC-5 | Every collection resolves an exact active plan local to its owning repository and truthfully records its build profile; a proposed, external-only, missing, version-mismatched, or falsely release-labelled plan/stack is refused. | Test (TC-129) |
| FR-024-AC-6 | No local generic evidence framework, foreign-runtime adapter, automated approval, or collapsed aggregate is introduced, and summaries retain every population before ratios without claiming release, qualification, certification, or monitor acceptance. | Test (TC-122, TC-125) |
