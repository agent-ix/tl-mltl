...
An admitted, fully evaluated complete lasso yields `proved` for the exact
trace subject only when every admitted possibility satisfies the claim, and
`refuted` for that trace only when every admitted possibility falsifies it.
Neither outcome widens to a transition/model claim. A model-wide `proved`
requires a procedure covering every admitted model trace; V1 has none and
returns `unsupported` for that subject kind. Both possible truth values yield
...
|---|---|---|
| FR-033-AC-1 | Trace-scoped proved, refuted and inconclusive are distinguished by independent complete and partial infinite cases, with subject scope and sound witness/counterexample details; one lasso cannot prove a model. | Test (TC-155, TC-156) |
