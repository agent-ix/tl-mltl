...
| FR-055-AC-4 | Campaign config refuses a Cargo-family member whose selected Rust identity, `RUSTC`, `PATH` resolution, or authored Cargo version disagree, and refuses duplicate procedure environment names; exact member environment overrides can bind stable and nightly members in one campaign. | Test (TC-200) |
