...
| NFR-003-AC-5 | Across semantic `jobs.*.steps[*].run` string scalars, independent of YAML spelling or style, a bare or path-qualified npm executable at command position, directly or through shell groups and a statically literal `sh`/`bash -c` script, using every documented npm-install alias (`install`, `add`, `i`, `in`, `ins`, `inst`, `insta`, `instal`, `isnt`, `isnta`, `isntal`, `isntall`) consumes exactly one executable ix-flow package, `@agent-ix/ix-flow@0.0.4`, and no other scoped, unscoped, alias-form, or identity-bearing alternate; npm- or shell-shaped data arguments are not commands, YAML metadata and YAML/shell comments do not change that population, a `#` after any started word remains argument content, the semantic trigger set is exactly [`workflow_dispatch`], and the local qualification gate observes `ix-flow --version` as exactly `0.0.4`. | Test (TC-036) |
