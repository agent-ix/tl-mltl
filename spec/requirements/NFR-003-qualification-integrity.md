...
|---|---|---|
| NFR-003-AC-1 | Every attested proof result is derived from the producer's own structured output; a producer whose output is absent, empty, or unreadable is an error naming the target that writes it, and never a pass. Every test that reads or temporarily mutates shared assurance inputs holds the repository's private serialization guard for the full access. | Test (TC-018, TC-019) |
