...
The hosted-workflow control reads the tracked workflow bytes, removes YAML
comment text beginning at an unquoted `#`, requires `workflow_dispatch` to
remain the only trigger, and inspects every remaining token naming an
`ix-flow@` package regardless of whether npm uses its `install` or `i` command
and long or short global option. Hash characters inside single- or double-quoted
tokens remain executable content rather than comment delimiters. It also
...
| NFR-003-AC-5 | After YAML comment text beginning at an unquoted `#` is removed, the hosted workflow contains exactly one executable `@agent-ix/ix-flow@0.0.4` package token and no other scoped, unscoped, or alias-form ix-flow package through any accepted npm install spelling; comment-only package spellings do not change that population, quoted hash characters remain part of their token, `workflow_dispatch` remains the only trigger, and the local qualification gate observes `ix-flow --version` as exactly `0.0.4`. | Test (TC-036) |
