...
| FND-306 | high | Resolved: CLI mapping revision/state follows the actual Git ref and checkout state, and the integration test compares both values to the identity embedded by the build contract rather than checking only shape. | NFR-002-AC-1 |
| FND-307 | medium | Resolved: the anchor set is derived and complete over every retained outer evidence manifest, including the current passing candidate. | NFR-002-AC-2 |
| FND-308 | medium | Resolved: tracked-source and resolved-Git-ref watches preserve exact identity without watching the in-tree Cargo target directory or missing worktree ref paths. | NFR-002-AC-1 |
| FND-309 | low | Nonzero Globally lower bounds are covered by the deterministic in-repo exhaustive oracle; retained external corpora remain unchanged because their bytes and provenance are independently pinned. | FR-001-AC-1, FR-005 |
