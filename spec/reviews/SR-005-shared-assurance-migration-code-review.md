...
| FREEZE | `schemas/tl-mltl-evidence-input-v1.schema.json` and `schemas/tl-mltl-evidence-manifest-v1.schema.json`, not deleted, and not in the same position as each other — see FND-506 |
...
| FND-506 | medium | `schemas/tl-mltl-evidence-input-v1.schema.json` on disk is not the bytes any retained envelope names: four records name `808fd9f3`, two name `d763369e`, the file is `7b7e4725`. ACCEPTED with the divergence pinned by TC-024. | schemas/README.md | implementation-bug-despite-evidence |
