...
| FND-1302 | medium | M21-02 FIXED for every shared-state helper. `chain_report`, `producer_shims`, `run_chain_with_path` and the store-isolation helper require a borrowed guard token, so deleting acquisition from an existing call site does not compile. | TC-019, NFR-003 | correct-requirement-no-evidence |
