...
| FND-3712 | medium | **SUPERSEDED by the closed grammar after review of `215745b`:** every unquoted redirection-bearing run script is rejected rather than partially scanned; non-`-c` shells still end only their command inspection, and the inert fixture exposes exact npm/bash argv words. | NFR-003-AC-5, TC-036, tests/shared_assurance.rs, tl-parse#28 review, tl-rewrite#34 review |
| FND-3713 | high | **FIXED by closing the admitted grammar after review of `bbdb93e`:** the scanner no longer attempts to tokenize `2>&1` or any other unquoted redirection. It rejects the full run script with its observed identities, and TC-036 includes the reviewer-discovered `2>&1> /dev/null` chain. | NFR-003-AC-5, TC-036, tests/shared_assurance.rs, tl-rewrite#34 review |
| FND-3714 | high | **FIXED after cross-lane review of `827ba93`:** double shell quotes do not suppress command-substitution grammar, so executable npm and redirection inside `$(...)` were treated as inert outer arguments. The closed grammar now rejects every unescaped `$` or backtick outside single quotes, while single-quoted and escaped spellings remain inert controls. | NFR-003-AC-5, TC-036, tests/shared_assurance.rs, tl-parse#30 review, tl-rewrite#34 review |
| FND-3715 | high | **FIXED from the sibling exact-head review of `89127ce`:** shell escaping was processed before GitHub-expression recognition, so `\${{ ... }}` could reach GitHub evaluation while remaining green locally. The scanner now recognizes `${{ ... }}` before shell quote or escape handling, with separate unquoted-escaped and double-quoted-escaped mutations. | NFR-003-AC-5, TC-036, tests/shared_assurance.rs, tl-parse#30 review, tl-mltl#43 review |
