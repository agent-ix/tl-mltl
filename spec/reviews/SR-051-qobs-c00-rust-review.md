...
| FND-5104 | medium | **FIXED:** Temporal compatibility delegates to `wire::request::derive`; TC-085 compares the complete result at default and exact-output limits, the complete one-short error, and direct/dispatch parity after independently lowering every `OwnerLimits` field. Repair/query compatibility takes only a closed selector, so TL cannot inspect, project, or accidentally retain either foreign artifact. | `src/wire/observation.rs::consume_temporal`; `src/wire/observation.rs::compatibility`; TC-085 |
...
| FND-5107 | high | **FIXED:** TL initially mirrored the QObs-owned repair/query wire labels and tested them against repeated local literals. QObs now exports the exact owner constants from the same values used by its encoders; TL aliases those constants and TC-085 compares the compatibility rows to the owner exports. | `quire_observation::authority::{repair,query}::CONTRACT`; `src/wire/observation.rs`; TC-085 |
...
documented, closed matches are exhaustive, and `Contract::label` delegates exact
foreign contract identity to owner-exported QObs constants.
