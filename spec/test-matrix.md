...
| NFR-003 | producer-boundary, shared-input serialization, state-vocabulary, mutation-probe, and hosted-tool identity tests | TC-018, TC-019, TC-022, TC-024, TC-036 | 🚧 planned |
...
| TC-036 | Require manual-only hosted CI to install exactly one scoped ix-flow 0.0.4 package token under every npm install spelling and require the local gate to observe that exact executable version | Integration | P0 | NFR-003-AC-5 | 🚧 planned |
