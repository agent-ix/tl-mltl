...
| TC-036 | Remove unquoted YAML comments before requiring manual-only hosted CI to contain exactly one executable scoped ix-flow 0.0.4 package token under every npm install spelling; retain quoted hashes, reject alias duplicates, and observe the exact local executable version | Integration | P0 | NFR-003-AC-5 | 🚧 planned correction |
