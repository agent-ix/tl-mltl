...
| TC-024 | Leave no local evidence framework or retained legacy evidence; enumerate every tracked and untracked-not-ignored repository path through Git, constrain exact denial, area, and declaration-exemption sets, scan raw bytes, and retain negative controls for preferred makefiles, non-UTF-8 content, unreadable paths, and enumeration failure | Integration | P0 | FR-006-AC-7 | ✅ implemented |
