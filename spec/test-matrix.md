...
| TC-036 | Select semantic job-step run scalars before enforcing one executable scoped ix-flow 0.0.4 package across the complete documented npm-install alias family and literal nested shells; ignore YAML metadata, distinguish true shell comments from started-word hashes, reject alternates, and observe the semantic manual-trigger set and exact runtime | Integration | P0 | NFR-003-AC-5 | ✅ implemented |
